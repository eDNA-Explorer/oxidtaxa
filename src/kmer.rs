...
/// Parsed spaced seed pattern. Precomputed for fast enumeration.
pub struct SpacedSeed {
    /// Original pattern string (e.g., "11011011011").
    pub pattern: String,
    /// Indices of '1' positions (e.g., [0, 1, 3, 4, 6, 7, 9, 10]).
    pub match_positions: Vec<usize>,
    /// Total span of the pattern (e.g., 11).
    pub span: usize,
    /// Weight = number of 1s = effective k-mer size for index computation.
    pub weight: usize,
}
...
/// Enumerate k-mers for a single sequence using a spaced seed pattern.
/// Window slides by 1 base but reads from seed.span-width window,
/// only using positions marked '1' in the pattern.
fn enumerate_single_spaced(
    seq: &[u8],
    seed: &SpacedSeed,
    pwv: &[i32],
    mask_reps: bool,
    ...
    let len = seq.len();
    if len < seed.span || seed.weight == 0 {
        return Vec::new();
    }

    let n_kmers = len - seed.span + 1;
    let mut result = vec![0i32; n_kmers];

    for j in 0..n_kmers {
        let mut sum = 0i32;
        let mut ambiguous = false;
        for (pw_idx, &pos) in seed.match_positions.iter().enumerate() {
            let base = base_to_index(seq[j + pos]);
            if base < 0 {
                ambiguous = true;
                break;
            }
            sum += base as i32 * pwv[pw_idx];
        }
        result[j] = if ambiguous { NA_INTEGER } else { sum };
    }

    // Masking uses seed.weight as the effective word_size
    if mask_reps {
        mask_repeats(&mut result, seed.weight);
    }
    if mask_lcrs {
        mask_simple(&mut result, seed.weight, 4, 20, 12.66667);
        mask_simple(&mut result, seed.weight, 4, 95, 38.90749);
    }
    if let Some(max_count) = mask_num {
        let tot = 4usize.pow(seed.weight as u32);
        mask_numerous(&mut result, max_count, tot, seed.weight);
    }
...
/// When `seed` is Some, uses spaced k-mer enumeration; otherwise contiguous.
pub fn enumerate_sequences(
    ...
    seed: Option<&SpacedSeed>,
) -> Vec<Vec<i32>> {
    if let Some(s) = seed {
        // Spaced seed path: pwv has length = seed.weight
        let w = s.weight;
        let mut pwv = vec![0i32; w];
        if w > 0 {
            if fast_moving_side {
                pwv[0] = 1;
                for i in 1..w {
        ...
        sequences
            ...
            .map(|(i, seq)| {
                ...
                enumerate_single_spaced(seq.as_bytes(), s, &pwv, mask_reps, mask_lcrs, mn)
            })
            .collect()
    } else {
        // Contiguous path (original behavior)
