...
fn filter_for_training(seqs: &[String], taxonomy: &[String]) -> (Vec<String>, Vec<String>) {
    ...
    for (i, seq) in seqs.iter().enumerate() {
        ...
        if full_tax.split("; ").count() < 4 || seq.len() < 30 {
            continue;
        }
...
fn main() {
    ...
    let below_half = model
        .fraction
        .iter()
        .filter(|f| matches!(f, Some(v) if *v < 0.03))
        .count();
    let at_max = model
        .fraction
        .iter()
        .filter(|f| matches!(f, Some(v) if (*v - 0.06).abs() < 1e-10))
        .count();
    println!("\nFraction distribution ({} nodes):", total_nodes);
    println!(
        "  At max (0.06):    {:>5} ({:.1}%)",
        at_max,
        100.0 * at_max as f64 / total_nodes as f64
    );
    println!(
        "  Below 0.03:       {:>5} ({:.1}%)",
        below_half,
        100.0 * below_half as f64 / total_nodes as f64
    );
    println!(
        "  At None (floor):  {:>5} ({:.1}%)",
        none_count,
        100.0 * none_count as f64 / total_nodes as f64
    );
    ...
    for ps in model.problem_sequences.iter().take(10) {
        ...
        println!(
            "  seq {:>4}: expected ...{:<30} predicted ...{}",
            ps.index, exp_short, pred_short
        );
    }
    ...
    for ps in &model.problem_sequences {
        let pred_depth = if ps.predicted.is_empty() {
            0
        } else {
            ps.predicted.split(';').filter(|s| !s.is_empty()).count()
        };
        if pred_depth < 20 {
            fail_depth_counts[pred_depth] += 1;
        }
    }
