...
use crate::matching::{int_match, parallel_match, vector_sum};
use crate::rng::RRng;
...
fn precompute(
    ...
    let raw_kmers = enumerate_sequences(seqs, k, false, false, &[], true);
    ...
    let rev_kmers = if !boths.is_empty() {
        ...
        let raw = enumerate_sequences(&rev_seqs, k, false, false, &[], true);
        raw.into_iter()
            .map(|v| {
                let mut sorted: Vec<i32> = v.into_iter()
                    .filter(|&x| x != NA_INTEGER).map(|x| x + 1).collect();
...
fn classify_one_pass(
    ...
    let above: Vec<usize> = confidences.iter().enumerate()
        .filter(|(_, &c)| c >= config.threshold).map(|(i, _)| i).collect();
