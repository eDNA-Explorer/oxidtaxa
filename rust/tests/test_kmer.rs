...
use idtaxa::kmer::{enumerate_sequences, NA_INTEGER};
...
fn test_kmer_standard_sequences() {
    ...
    let result = enumerate_sequences(&seqs, 8, false, false, &[], true);
...
fn test_kmer_different_k_values() {
    ...
    for k_val in [1, 3, 5, 8, 10, 13, 15] {
        ...
        let result = enumerate_sequences(&seqs, k_val, false, false, &[], true);
...
fn test_kmer_masking_repeats() {
    ...
    let result = enumerate_sequences(&seqs, 8, true, false, &[], true);
    for (i, (got, expected)) in result.iter().zip(golden.iter()).enumerate() {
        assert_eq!(
            got, expected,
...
fn test_kmer_masking_lcr() {
    ...
    let result = enumerate_sequences(&seqs, 8, false, true, &[], true);
    for (i, (got, expected)) in result.iter().zip(golden.iter()).enumerate() {
        assert_eq!(
            got, expected,
...
fn test_kmer_masking_both() {
    ...
    let result = enumerate_sequences(&seqs, 8, true, true, &[], true);
    for (i, (got, expected)) in result.iter().zip(golden.iter()).enumerate() {
        assert_eq!(
            got, expected,
